...
int* shrink_array(int* myarray, int size) {
  

    int* shrunk_array = new int[size - 1];
    for (int i = 0; i < size - 1; i++) {
        shrunk_array[i] = myarray[i];
    }
...
int main() {
    ...
    // STEP 07 COMPLETED
    cout << "STEP 07 COMPLETED." << endl << endl;
    int* shrunk_array = shrink_array(myarray, records);
    int shrunk_size = records - 1;

    cout << "Shrunk array elements (excluding the last element) are given as:" << endl;
    for (int i = 0; i < shrunk_size; i++) {
        cout << shrunk_array[i] << "  " << endl;
    }
